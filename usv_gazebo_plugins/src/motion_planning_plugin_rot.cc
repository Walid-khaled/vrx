...
namespace gazebo
{
  class ModelMotion_rot : public ModelPlugin
  {
    public: void Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/)
    {
      ...
      this->updateConnection = event::Events::ConnectWorldUpdateBegin(
          std::bind(&ModelMotion_rot::OnUpdate, this));
      
      this->old_secs =ros::Time::now().toSec();
      
      // Create a topic name
      std::string frequancy_topicName_rot = "/frequancy_rot";
      std::string magnitude_topicName_rot = "/magnitude_rot";
      ...
      ros::SubscribeOptions so =
        ros::SubscribeOptions::create<std_msgs::Float32>(
            frequancy_topicName_rot,
            1,
            boost::bind(&ModelMotion_rot::OnRosMsg, this, _1),
            ros::VoidPtr(), &this->rosQueue);
      this->rosSub = this->rosNode->subscribe(so);
      
      // Spin up the queue helper thread.
      this->rosQueueThread =
        std::thread(std::bind(&ModelMotion_rot::QueueThread, this));
        
        
      // Magnitude
      ros::SubscribeOptions so2 =
        ros::SubscribeOptions::create<std_msgs::Float32>(
            magnitude_topicName_rot,
            1,
            boost::bind(&ModelMotion_rot::OnRosMsg_Magn, this, _1),
            ros::VoidPtr(), &this->rosQueue2);
      this->rosSub2 = this->rosNode->subscribe(so2);
      
      // Spin up the queue helper thread.
      this->rosQueueThread2 =
        std::thread(std::bind(&ModelMotion_rot::QueueThread2, this));
    ...
    public: void OnUpdate()
    {
      ...
      if (this->rot_axis_freq != 0.0)
      {
        max_delta = 1.0 / this->rot_axis_freq;
      }
      
      double magnitude_speed = this->rot_axis_magn;
      ...
      this->model->SetLinearVel(ignition::math::Vector3d(0.2*speed, 0.2*speed, 0));
      this->model->SetAngularVel(ignition::math::Vector3d(0, 0, 0.2*speed));
    }
    
    
    public: void SetFrequency(const double &_freq)
    {
      this->rot_axis_freq = _freq;
      ROS_WARN("rot_axis_freq >> %f", this->rot_axis_freq);
    }
    
    public: void SetMagnitude(const double &_magn)
    {
      this->rot_axis_magn = _magn;
      ROS_WARN("rot_axis_magn >> %f", this->rot_axis_magn);
    }
    ...
    int direction = 1;
    // Frequency 
    double rot_axis_freq = 1;
    // Magnitude of the Oscilations
    double rot_axis_magn = 30.0;
  ...
  GZ_REGISTER_MODEL_PLUGIN(ModelMotion_rot)
}
